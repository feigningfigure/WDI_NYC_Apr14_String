...
# # lines.each {|x| x[10].split("-")}
# lines = lines_data[0] {|x| x[10].split("-")}
# lines.each do |x|
#   single_line = SubwayLine.find_by_name(x)
#   line_id = single_line.id
#   line_stops = single_line.stops.all
#   line_stops.each do |x|
#     stop_id = x.id
# Stop.create(
